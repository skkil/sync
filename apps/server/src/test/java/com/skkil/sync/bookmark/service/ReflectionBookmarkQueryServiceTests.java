...
@Import({
  ...
  ReflectionBookmarkQueryRepository.class,
  ReflectionBookmarkQueryService.class,
  BookmarkedReflectionCursorPaginationProvider.class,
  PaginationService.class,
  CursorConverter.class,
  ReflectionBookmarkQueryServiceTests.TestConfig.class
})
class ReflectionBookmarkQueryServiceTests {
  ...
  @Autowired private ReflectionBookmarkQueryRepository reflectionBookmarkQueryRepository;
  @Autowired private ReflectionBookmarkQueryService reflectionBookmarkQueryService;
  @Autowired private BookmarkedReflectionCursorPaginationProvider paginationProvider;
  @Autowired private MediaDomainService mediaDomainService;

  @BeforeEach
  void setUp() {
    dsl.execute(
        "TRUNCATE TABLE reflection_bookmarks, reflections, media_files, users RESTART IDENTITY CASCADE");
    ...
    insertReflection(101L, "newest", AUTHOR_WITH_IMAGE_ID);
    insertReflection(102L, "tie-high", AUTHOR_WITHOUT_IMAGE_ID);
    insertReflection(103L, "tie-low", AUTHOR_WITHOUT_IMAGE_ID);
    insertReflection(104L, "oldest", AUTHOR_WITHOUT_IMAGE_ID);
    insertReflection(105L, "other-user", AUTHOR_WITH_IMAGE_ID);
  ...
  @Test
  @DisplayName("[getBookmarkedReflections] 북마크 생성일/ID 기준 cursor pagination")
  void getBookmarkedReflections_paginatesByBookmarkCreatedAtAndId() {
    var firstPage =
        paginationService.paginate(
            reflectionBookmarkQueryRepository.getBookmarkedReflections(REQUESTER_ID),
            paginationProvider,
  ...
  @Test
  @DisplayName("[getBookmarkedReflections] 북마크 목록 응답 매핑")
  void getBookmarkedReflections_mapsBookmarkedPosts() throws Exception {
    ...
    var response =
        reflectionBookmarkQueryService.getBookmarkedReflections(
  ...
  private void insertReflection(Long id, String slug, Long authorId) {
    dsl.insertInto(REFLECTIONS)
        .set(REFLECTIONS.ID, id)
        .set(REFLECTIONS.CREATED_AT, CREATED_AT)
        .set(REFLECTIONS.UPDATED_AT, CREATED_AT)
        .set(REFLECTIONS.SLUG, slug)
        .set(REFLECTIONS.AUTHOR_ID, authorId)
        .set(REFLECTIONS.CONTENT, slug + " content")
        .execute();
  }

  private void insertBookmark(Long id, Long userId, Long reflectionId, LocalDateTime bookmarkedAt) {
    dsl.insertInto(REFLECTION_BOOKMARKS)
        .set(REFLECTION_BOOKMARKS.ID, id)
        .set(REFLECTION_BOOKMARKS.CREATED_AT, bookmarkedAt)
        .set(REFLECTION_BOOKMARKS.UPDATED_AT, bookmarkedAt)
        .set(REFLECTION_BOOKMARKS.USER_ID, userId)
        .set(REFLECTION_BOOKMARKS.REFLECTION_ID, reflectionId)
        .execute();
  }
  ...
  static class TestConfig {
    ...
    ReflectionBookmarkMapper reflectionBookmarkMapper() {
      return new ReflectionBookmarkMapper() {};
    }
